<?php
    ...
    if($status == 0){ 
        $message = "Adopted";
        $colorClass = "red-text";
        $disabled = "disabled"; // Disable the button if status is 0 (adopted)
    }else if($status == 2){ 
        $message = "Requested";
        $colorClass = "red-text";
        $disabled = ""; // Enable the button if status is 2 (requested)
    } else {
        $message = "Available";
        $colorClass = "green-text";
        $disabled = ""; // Enable the button if status is neither 0 nor 2
    }
?>
...
<body>
    ...
    <div class="d-flex flex-row gap-5 flex-wrap justify-content-start align-items-center p-5">
        ...
        <div class="w-40 ps-0">
            ...
            <button class='btn text-white' id='upBtn' <?= $disabled ?>> <a class='text-decoration-none text-white' href='adopt.php?x=<?= $row["id"] ?>'>Take me home </a> </button>
        </div>
